			...
			Player *current_player, Cell *cells)
{
	int	i;
	int	selected_player;
	int	total_stones_on_board;
	int	max_number_of_stones_to_select;
	int	number;

	selected_player = 0;
	total_stones_on_board = PLAYER_ONE->number_of_stones_on_board
		+ PLAYER_TWO->number_of_stones_on_board;
	max_number_of_stones_to_select = total_stones_on_board < dice ?
		total_stones_on_board : dice;
	number = max_number_of_stones_to_select == 1 ? 1
		: select_number_of_stones_for_water(max_number_of_stones_to_select,
		current_player);
	...
	while (++i < number)
	{
		...
		selected_player == 1 ? remove_stone_from_board(cells, ABILITY_WATER,
			level, PLAYER_ONE, current_player)
			: remove_stone_from_board(cells, ABILITY_WATER, level,
